...
headline models (OLS, Random Forest, COMB). The pipeline also runs the Day-3/4 grading diagnostics: feature-search logging,
correlation/VIF checks, transaction costs/turnover/gross-vs-net performance,
...
from backtest_portfolio import build_costed_portfolio_outputs  # noqa: E402
from diagnostics import run_residual_diagnostics  # noqa: E402
from feature_diagnostics import run_feature_diagnostics_from_frame  # noqa: E402
from feature_search_log import write_feature_search_log  # noqa: E402
...
def run_window(model_df_window: pd.DataFrame, cfg: Config, output_dir: Path, tag: str, label: str,
               input_path: Path, input_sha256: str, cost_bps: float):
    ...
    # Day 4 required evaluation: explicit turnover/cost assumptions and gross-vs-net.
    portfolio_diag_dir = output_dir / "diagnostics" / "portfolio"
    _, cost_summary = build_costed_portfolio_outputs(
        predictions, portfolio_diag_dir, tag,
        cost_bps=cost_bps, top_frac=0.20, min_leg_names=3,
    )

    # Day 4 required residual diagnostics. Models predict standardized y, so residual=y-prediction.
    residual_diag_dir = output_dir / "diagnostics" / "residuals"
    residual_summary, _ = run_residual_diagnostics(
        predictions, residual_diag_dir, tag,
    )
    ...
    write_run_manifest(
        ...
        artifacts={
            ...
            "portfolio_performance_summary": perf_path,
            "model_metrics": output_dir / f"{tag}_model_metrics.csv",
            "gross_net_summary": portfolio_diag_dir / f"{tag}_portfolio_gross_net_summary.csv",
            "residual_diagnostics": residual_diag_dir / f"{tag}_residual_diagnostics_summary.csv",
        },
    ...
    print(perf_summary[perf_summary["leg"] == "long_short"]
          ...
          .round(4).to_string(index=False))
    print(f"    Net-of-cost performance ({cost_bps:g} bps one-way):")
    net_cols = ["model", "ann_return", "ann_vol", "sharpe", "max_drawdown",
                "avg_daily_traded_notional", "total_transaction_cost"]
    print(cost_summary[cost_summary["return_basis"] == "net"][net_cols]
          .round(4).to_string(index=False))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cost-bps", type=float, default=10.0,
                        help="one-way transaction cost in bps per dollar traded (default: 10)")
...
def main():
    ...
    # Day 3 required research evidence and multicollinearity diagnostics.
    feature_diag_dir = out_root / "diagnostics" / "features"
    print("      writing feature-search log and correlation/VIF diagnostics...")
    write_feature_search_log(feature_diag_dir / "feature_search_log.csv")
    run_feature_diagnostics_from_frame(
        model_df_all,
        feature_diag_dir,
        max_vif_rows=50_000,
        corr_threshold=0.80,
    ...
    metrics_model, perf_model = run_window(
        ...
        input_path=args.input, input_sha256=input_sha256, cost_bps=args.cost_bps,
    )
